...
class FinishResult:
    ...
    def summary(self) -> str:
        ...
        if self.synced_to:
            lines.append(f"Скопировано в: {self.synced_to}")
        return "\n".join(lines)
...
def _sync_basis_scripts(json_path: Path) -> Path | None:
    """examples/foo.json → basis_scripts/foo.json"""
    root = json_path.resolve().parent.parent
    if json_path.parent.name != "examples":
        return None
    dest = root / "basis_scripts" / json_path.name
...
def finish_project(
    ...
    *,
    ...
    if result.ok and sync_basis_scripts:
        result.synced_to = _sync_basis_scripts(path)
