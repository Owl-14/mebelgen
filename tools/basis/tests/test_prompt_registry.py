...
def test_operation_contract_is_deterministic_and_protects_schema_and_part_scope() -> None:
    edited = apply_prompt_operations(SPEC, [
        {"op": "set", "path": "/dimensions/depth", "value": 600},
    ], node="edit_operations")
    assert edited["dimensions"]["depth"] == 600
    assert SPEC["dimensions"]["depth"] != 600

    with pytest.raises(ValueError, match="outside mutable"):
        apply_prompt_operations(SPEC, [
            {"op": "set", "path": "/schemaVersion", "value": "other"},
        ], node="edit_operations")
    with pytest.raises(ValueError, match="only mutate overrides"):
        apply_prompt_operations(SPEC, [
            {"op": "set", "path": "/dimensions/width", "value": 1},
        ], node="part_edit")
