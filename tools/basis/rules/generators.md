...
## composite
`blocks[]`: `{name, origin:{x,y,z}, spec: <вложенный ParamSpec>}`. Генератор строит
каждый блок и сдвигает на origin. `origin.y` — вертикальная постановка (верхние
навесные над нижними: кухни, стеллаж-над-тумбой). Вложенность — один уровень.
