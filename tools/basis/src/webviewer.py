...
def viewer_payload(project: dict[str, Any], *, include_holes: bool = True) -> dict[str, Any]:
    ...
    return {"panels": panels, "colors": _COLORS, "holes": holes,
...
function MebelScene(container){
  ...
  function setPayload(DATA,opts){
    ...
    PANELS.forEach((p,i)=>{
      ...
      const col=new THREE.Color(COLORS[p.type]||'#c9a06a'); col.offsetHSL(0,0,((i%5)-2)*0.009);
      ...
      const e=edge(mesh); holder(gi).add(e);
    });
    HW.forEach((h,j)=>{
      const gi=ownH[j];
