...
ХВОСТ (эксперименты 2026-09-11, rules/local_b3d_pipeline.md): 64 байта после
zlib-секции — подпись содержимого формата версии 17 (Header.Version 17,
Document 25.17), которую пишет облако; без неё или при изменённом содержимом
Просмотр даёт «Ошибка чтения файла». Формат версии 15 (десктопный Мебельщик
2023.10: Header.Version 15, Document 23.15) подписи не имеет: такой файл
воспроизводится байт-в-байт (zlib уровня 6) и открывается после любых правок.
Значит:
- ЧТЕНИЕ .b3d — полноценное (parse_b3d) из ЛЮБОГО файла БАЗИС/облака;
- ЗАПИСЬ — write_b3d пишет структурно точное дерево; сборка нативного файла из
  project.json — b3d_builder.py (формат 15, без хвоста), открываемость проверяет
  b3d_viewer_check.py через БАЗИС-Просмотр.
"""
...
def parse_b3d(data: bytes) -> dict[str, Any]:
    ...
    trailer — непрозрачный хвост после zlib-потока (может отсутствовать; сохраняем как есть)."""
    if data[:4] != MAGIC:
    ...
    while p < len(data):
        ...
        if flag == 0x01:                                  # zlib-секция
            ...
            p += consumed
            trailer = data[p:]                            # непрозрачный хвост после потока
            break
...
def write_b3d(sections: list[tuple[int, tuple]], trailer: bytes = b"") -> bytes:
    """Собрать BZ85-контейнер из секций и непрозрачного trailer без обещания openability."""
    out = bytearray(MAGIC)
    for flag, root in sections:
        body = _serialize_section([root])
        out += SECTION + bytes([flag])
        # уровень 6 (zlib по умолчанию) воспроизводит десктопный файл байт-в-байт;
        # уровень 9 давал другой поток и мешал сравнивать с оригиналом
        out += zlib.compress(body, 6) if flag == 0x01 else body
    out += trailer
    return bytes(out)
