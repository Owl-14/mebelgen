...
ГРАНИЦА (проверено, task 12644 + эксперименты REPACK/ZTAIL): у проверенного
облачного файла после zlib-секции есть 64-байтный непрозрачный хвост. После его
обнуления БАЗИС-Просмотр дал «Ошибка чтения файла». Однако подтверждённый
desktop/production fixture ``wardrobe_demo_production.b3d`` хвоста не имеет.
Следовательно, роль хвоста и правила разных производителей B3D до конца не
доказаны; называть его обязательной подписью любого нативного файла нельзя.
Значит:
...
- write_b3d даёт структурно корректное дерево, но открываемость пересобранного
  файла не подтверждена. Производственный .b3d по-прежнему сохраняет облако или
  лицензированный desktop БАЗИС. См. Linear AKD-98 и rules/local_b3d_pipeline.md.
"""
