...
_MUTATION_CAPABILITY: dict[str, Any] = {
    "type": "object",
...
def capability_schema(node: str) -> dict[str, Any]:
    ...
    if node in {"edit_operations", "repair"}:
        result = copy.deepcopy(_MUTATION_CAPABILITY)
        schema = json.loads(PARAMSPEC_SCHEMA_PATH.read_text(encoding="utf-8"))
    ...
    if node == "part_edit":
        result = copy.deepcopy(_MUTATION_CAPABILITY)
        result["properties"]["operations"]["items"]["properties"]["path"] = {
            "type": "string", "pattern": "^/overrides(?:/|$)"
        }
        schema = json.loads(PARAMSPEC_SCHEMA_PATH.read_text(encoding="utf-8"))
        result["x-override-item-schema"] = _compact_schema(
            (schema.get("$defs") or {}).get("Override") or {}
        )
        return result
...
def _pointer_parts(path: str) -> list[str]:
    if not isinstance(path, str) or not path.startswith("/"):
        raise ValueError("operation path must be a JSON pointer")
    return [part.replace("~1", "/").replace("~0", "~") for part in path[1:].split("/")]


def apply_prompt_operations(spec: Mapping[str, Any], operations: list[dict[str, Any]],
                            *, node: str) -> dict[str, Any]:
    """Apply the small mutation contract and reject paths outside ParamSpec."""
    if node not in {"edit_operations", "part_edit", "repair"}:
        raise ValueError(f"node {node!r} cannot mutate ParamSpec")
    if not isinstance(operations, list) or len(operations) > 24:
        raise ValueError("operations must be a list with at most 24 items")
    schema = json.loads(PARAMSPEC_SCHEMA_PATH.read_text(encoding="utf-8"))
    allowed_roots = set(_allowed_param_roots(schema))
    result: Any = copy.deepcopy(dict(spec))
    for operation in operations:
        if not isinstance(operation, dict) or operation.get("op") not in {"set", "remove"}:
            raise ValueError("unsupported prompt operation")
        parts = _pointer_parts(operation.get("path", ""))
        if not parts or parts[0] == "schemaVersion" or parts[0] not in allowed_roots:
            raise ValueError("operation path is outside mutable ParamSpec fields")
        if node == "part_edit" and parts[0] != "overrides":
            raise ValueError("part edit may only mutate overrides")
        parent = result
        for position, part in enumerate(parts[:-1]):
            if isinstance(parent, list):
                index = int(part)
                if index < 0 or index >= len(parent):
                    raise ValueError("operation list index is out of range")
                parent = parent[index]
            elif isinstance(parent, dict):
                if part not in parent:
                    if operation["op"] == "set":
                        next_part = parts[position + 1]
                        parent[part] = [] if next_part == "-" or next_part.isdigit() else {}
                    else:
                        raise ValueError("operation path does not exist")
                parent = parent[part]
            else:
                raise ValueError("operation path crosses a scalar")
        leaf = parts[-1]
        if isinstance(parent, list):
            if leaf == "-" and operation["op"] == "set":
            ...
            raise ValueError("operation target is not a container")
    return result
