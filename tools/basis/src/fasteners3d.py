...
# Условные цвета крепежа (как в библиотеке технолога, реверс
# wardrobe_demo_production: Color 255 → красный, 16711680 → синий):
# красный — конфирматы/саморезы/SE01PB, синий — эксцентриковые стяжки.
_RED = (0.85, 0.08, 0.08)
_BLUE = (0.10, 0.15, 0.85)

# purpose → (kind, цвет Kd для .mtl)
_PURPOSE_KIND = {
    "короб ящика (саморез)": ("screw", _RED),
    "направляющая (винт)": ("screw", _RED),
    "направляющая (саморез)": ("screw", _RED),
    "петля (планка)": ("screw", _RED),
    "петля (накол чашки)": ("screw", _RED),
    "ручка (винт)": ("hscrew", (0.82, 0.84, 0.85)),
    "задник (гвоздь)": ("nail", (0.60, 0.63, 0.65)),
    "задник (саморез)": ("screw", _RED),
    "полкодержатель": ("shelfpin", (0.82, 0.84, 0.85)),
    "эксцентрик полки (чашка Ø20)": ("cam20", _RED),
    "эксцентрик полки (шток)": ("pin", _RED),
    "евровинт (проход Ø8)": ("confirmat", _RED),
    "конфирмат": ("confirmat", _RED),
    "петля (чашка Ø35)": ("cup", (0.55, 0.57, 0.60)),
    "эксцентрик (чашка Ø15)": ("cam", _BLUE),
    # «эксцентрик (шток)» + «эксцентрик (канал Ø8)» собираются парой → bolt
    "фасадная стяжка (эксцентрик Ø15)": ("cam", _BLUE),
    "фасадная стяжка (шток)": ("bolt", _BLUE),
    "замок (цилиндр Ø18)": ("lock", (0.82, 0.84, 0.85)),
    "штангодержатель (саморез)": ("screw", _RED),
    "опора (саморез)": ("screw", _RED),
...
_BODY_KINDS = {
    ...
    # полозья направляющих (паритет с файлом технолога: Obj 4004 → 2 полоза;
    # у нас — те же боксы, что рисует Studio, механикой objType 5)
    "guide_corpus": ("Направляющая шариковая (корпусный полоз)",
                     "направляющие шариковые", (0.72, 0.74, 0.77), "box"),
    "guide_drawer": ("Направляющая шариковая (ящичный полоз)",
                     "направляющие шариковые", (0.72, 0.74, 0.77), "box"),
}
...
def build_fastener_objects(holes: list[dict[str, Any]],
    ...
    for i, h in enumerate(hole_list):
        ...
        if mate is not None:
            used.add(mate)
        v = _vec(h)
        _add("bolt", _BLUE, h["depth"], 8, (h["x"], h["y"], h["z"]), v,
             [{"pos": {"x": 0, "y": 0, "z": 0}, "dir": {"x": 0, "y": 0, "z": -1},
               "depth": h["depth"], "diameter": h["diameter"]},
              {"pos": {"x": 0, "y": 0, "z": 0}, "dir": {"x": 0, "y": 0, "z": 1},
               "depth": 34, "diameter": 8}])
    ...
    for i, h in enumerate(hole_list):
        ...
        v = _vec(h)
        _add("confirmat", _RED, 50, 7, (h["x"], h["y"], h["z"]), v,
             [{"pos": {"x": 0, "y": 0, "z": 0}, "dir": {"x": 0, "y": 0, "z": -1},
