...
def rollout_provider() -> str | None:
    """Return the paid default only while the explicit rollout flag is on."""
    if not truthy_env("SPEC_CHAT_PAID_ENABLED"):
        return None
    provider = os.environ.get("SPEC_CHAT_PAID_PROVIDER", "glm-5.2").strip().casefold()
    if provider not in PAID_MODELS:
        raise ValueError(f"Unknown SPEC_CHAT_PAID_PROVIDER={provider!r}")
    return provider


def paid_calls_allowed(provider_id: str) -> None:
    """Fail closed in CI and unless the production rollout is explicit."""
...
def request_budget(provider_id: str, approximate_input_tokens: int) -> dict[str, Any]:
    """Bound completion size and reject a request whose worst case is too costly."""
    ...
    estimate = policy.estimated_cost_usd(approximate_input_tokens, output_tokens)
    if ceiling <= 0 or estimate > ceiling:
        raise RuntimeError(
            f"Paid LLM request estimate ${estimate:.4f} exceeds ${max(0, ceiling):.4f} limit"
        )
    return {"max_completion_tokens": output_tokens, "estimated_cost_usd": estimate}
...
__all__ = [
    ...
    "rollout_provider", "truthy_env", "usage_cost",
]
