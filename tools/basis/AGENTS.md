...
Сначала применяются обязательные правила всего репозитория из
[`../../AGENTS.md`](../../AGENTS.md), включая работу в отдельных ветках,
защиту чужих изменений, тестовые гейты и запрет несанкционированного deploy.
Этот файл добавляет предметные правила `tools/basis`.

## Обязательный runbook при изменении движка

**До первого изменения обязательно полностью прочитай
[`rules/engine-change-runbook.md`](rules/engine-change-runbook.md)**, если задача
касается хотя бы одной части мебельного движка: ParamSpec или его схемы,
AI-маршрутизации и промптов, typed operations, генераторов и геометрии,
материалов, присадок и фурнитуры, производственных проверок/экспорта, Studio API,
3D-viewer, трассировки, тестовых ParamSpec/golden-файлов либо миграции каталога.

Это правило действует и для «маленькой» правки одного поля. Если изменение
меняет контракт, поток данных, безопасные границы или процедуру релиза — в той
же задаче обнови runbook. Не начинай реализацию, опираясь только на промпт или
на описание задачи.
