...
чтобы не накладываться, и удерживаются в полосе [top, bottom]. Используется для
колонок выносок слева/справа и для размерных подписей.
"""
...
def stack(labels: list[Label], top: float, bottom: float, gap: float = 4.0) -> list[Label]:
    ...
    Возвращает тот же список с проставленным .y (стабильный порядок входа сохранён).
    """
    if not labels:
        return labels
    ...
    for lab in order:
        ...
        lab.y = y
        cursor = y + half + gap
    # если перелезли за нижнюю границу — поджать весь столбец вверх
    overshoot = (order[-1].y + order[-1].height / 2) - bottom
    if overshoot > 0:
        for lab in order:
            lab.y -= overshoot
        # подровнять, чтобы не вылезти за top
...
def overlaps(labels: list[Label], gap: float = 0.0) -> int:
    """Сколько пар меток перекрываются по Y (для самопроверки раскладки)."""
